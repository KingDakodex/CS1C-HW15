...
int main()
{
    ...
    /*
    * modified as the double functions, prerovided, were causing a crash
    * 
    ...
    */
    ...
    double* lowa = low(&d[0], &d[9]); // min of array
    cout << "low alg: double array min value is " << *lowa << endl << endl;

    double* lowv1 = low(&v[0], middle);          // min of vector first half
    double* lowv2 = low(middle, &v[0]+v.size()); // min of vector second half
...
An stl container is... a class whose purpose is to conatian other objects.
Example: Vector, list, set, etc.

An stl iterator is... the mechanism that makes it possible to decouple algorithms from containers.
Example: the find algorithm takes two iterators as its arguments one for the begining of the container and one fore the end.

An stl algorithm is... a function that manipulates data stored in containers to various ends.
Example: Reverse, which 'reverses' the data stored in an array or other conatiner.

*/
// Q#2
/*

The iterator architecture allows decoupling from algorithms by... allowing substitions types in its arguments 
    (ie int* double* etc rather than specificaly only working for one type).
This is important because... it allows code flexibility and reusability.
